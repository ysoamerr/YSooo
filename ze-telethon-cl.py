...
#logo
logo = pyfiglet.figlet_format('*      𝐌𝐎      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == '𝐌𝐎':
        await bot.send_message(OWNER_ID, f"""**∘ بوت 𝐌𝐈𝐊𝐎  لتجميع النقاط واوامر اخرى 

∘ مطور ومبرمج البوت يسو الدليمي - @I_RA6

∘ اصدار البوت = الاخير و منقح**""")
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if answer.text == 'نعم':
            ...
            for file in os.listdir():
                if file not in ['run.py', '𝐌𝐎kalb.py', '𝐌𝐎.json', '__pycache__', '𝐌𝐎-telethon-cl.py', 'bot.session']:
                    os.remove(file)
        elif answer.text == 'لا':
...
async def callback(event):
    ...
    async with bot.conversation(event.sender_id) as conv:
        ...
        await conv.send_message("**✪ تم حضر البوت بنجاح **")
        for user_id in stored_users:
            try:
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ 🇮🇶 𝐌𝐈𝐊𝐎 🇮🇶 𝐁𝐎𝐓 ༻', 'button0')]
        ]
    ...
    async with bot.conversation(event.sender_id) as conv:
        ...
        try: 
            ...
            if not client.is_connected():
                ...
                return
            if not await client.is_user_authori𝐌𝐎d():
